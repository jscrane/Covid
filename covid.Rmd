...
Statistics on testing are only published weekly. However, extrapolating the last [figure](https://www.rte.ie/news/coronavirus/2020/0317/1123663-coronavirus-ireland/) with the latest number of cases gives a rough estimate:
...
m <- merge(d, t, by="date")
lm <- tail(m, 1)
ld$total_cases * lm$tests / lm$total_cases
```
